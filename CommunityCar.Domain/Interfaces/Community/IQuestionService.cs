...
{
    Task<Question> CreateQuestionAsync(string title, string content, Guid authorId, string? tags = null);
    Task<Question> UpdateQuestionAsync(Guid questionId, string title, string content, string? tags = null);
    Task DeleteQuestionAsync(Guid questionId);
    Task<QuestionDto?> GetQuestionByIdAsync(Guid questionId);
    Task<PagedResult<QuestionDto>> GetQuestionsAsync(QueryParameters parameters, string? searchTerm = null, string? tag = null, bool? isResolved = null);
    Task<PagedResult<QuestionDto>> GetUserQuestionsAsync(Guid userId, QueryParameters parameters);
    Task<PagedResult<QuestionDto>> GetTrendingQuestionsAsync(QueryParameters parameters, string? searchTerm = null, string? tag = null);
    Task<PagedResult<QuestionDto>> GetRecentQuestionsAsync(QueryParameters parameters, string? searchTerm = null, string? tag = null);
